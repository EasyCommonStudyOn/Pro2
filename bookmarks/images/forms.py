...
"""
Мы определили форму ModelForm из модели Image, включая только поля title,
url и description. Пользователи не будут вводить URL-адрес изображения
прямо в форму. Вместо этого мы предоставим им инструмент JavaScript, ко-
торый позволит им выбирать изображение с внешнего сайта, а форма будет
получать URL-адрес изображения в качестве параметра. Новый экземпляр изображения создается путем вызова метода save()
...
"""
Здесь мы определили метод clean_url(), чтобы очищать поле url. Исходный
